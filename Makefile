...
build_logservice_container:
	docker build --tag logservice:${GIT_TAG} --file app/cmd/logservice/Dockerfile --build-arg TARGETARCH=amd64 .
	docker image tag logservice:${GIT_TAG} logservice:latest

build_registryservice_container:
	docker build --tag registryservice:${GIT_TAG} --file app/cmd/registryservice/Dockerfile --build-arg TARGETARCH=amd64 .
	docker image tag registryservice:${GIT_TAG} registryservice:latest

build_gradingservice_container:
	docker build --tag gradingservice:${GIT_TAG} --file app/cmd/gradingservice/Dockerfile --build-arg TARGETARCH=amd64 .
	docker image tag gradingservice:${GIT_TAG} gradingservice:latest

build_teacherportalservice_container:
	docker build --tag teacherportalservice:${GIT_TAG} --file app/cmd/teacherportal/Dockerfile --build-arg TARGETARCH=amd64 .
